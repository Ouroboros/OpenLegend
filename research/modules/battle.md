...
- `battle-closure.tsv`：0项`pending_mapping`、0项`pending_implementation`、42项`implemented_pending_review`、39项`platform_adapted`；统一最终REVIEW为39/81。
- B7 `sub_2DE03`联合边界已随typed battle result回送推进到`implemented_pending_review`。
- 下一停点：按`audit_order=40`审计`sub_36E7F`目标寻路图handler；实现/测试通过不替代逐owner最终汇编↔C++ REVIEW门禁。
