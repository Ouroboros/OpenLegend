...
7. `sub_3AA17` 等待队尾重排为 `implemented_pending_review`；`sub_3AA4B` 已锁定重绘→present→置自动flag→AI的顺序；`sub_3AA85` 已恢复双32×32 pass、地形/overlay/物件/角色/effect/damage的typed命令计划，`BattleRenderer`已按WDX/WMP、EFT、动态FIGHT pointer基址实际执行RLE、高亮、CLOUD混色overlay和damage字体像素，独立资产oracle与C++整帧hash一致；BattleSession已用于初始战场、actor-present、玩家动作菜单、movement路径光标和每个玩家/AI移动步的重绘/present，targeting/动画/结果调用点仍待接入；
...
- `battle-closure.tsv`：0项 `pending_mapping`、35项 `pending_implementation`、46项 `implemented_pending_review`，0项最终关闭。
- B7 尚余 `sub_2DE03/sub_31C75` 联合边界；只有 battle result 实际回送 scene 后才能把两项推进到 `implemented_pending_review`。
- 下一停点：接入玩家targeting与攻击/用毒/解毒/医疗动作，再执行AI恢复后的攻击/支持/物品/暗器效果，并把结果回送scene边界。
