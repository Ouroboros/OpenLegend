...
- `battle-closure.tsv`：0项`pending_mapping`、0项`pending_implementation`、41项`implemented_pending_review`、40项`platform_adapted`；统一最终REVIEW为40/81。
- B7 `sub_2DE03`联合边界已随typed battle result回送推进到`implemented_pending_review`。
- 下一停点：按`audit_order=41`审计`sub_36EF8`移动阻挡图初始化；实现/测试通过不替代逐owner最终汇编↔C++ REVIEW门禁。
