...
- `battle-closure.tsv`：0项 `pending_mapping`、27项 `pending_implementation`、54项 `implemented_pending_review`，0项最终关闭。
- B7 尚余 `sub_2DE03/sub_31C75` 联合边界；只有 battle result 实际回送 scene 后才能把两项推进到 `implemented_pending_review`。
- 下一停点：接玩家攻击targeting/type1方向与实际FIGHT/EFT/damage/commit continuation，再接物品选择/物品continuation，复用同步相位完成AI对应效果，并把结果回送scene边界。
