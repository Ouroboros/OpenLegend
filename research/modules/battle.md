...
- `battle-closure.tsv`：14项 `pending_mapping`、28项 `pending_implementation`、39项 `implemented_pending_review`，0项最终关闭。
- B7 尚余 `sub_2DE03/sub_31C75` 联合边界；只有 battle result 实际回送 scene 后才能把两项推进到 `implemented_pending_review`。
- 下一停点：恢复`sub_361AC/sub_36209`请求handler，再推进`sub_36210/sub_363AC`其他动作与`sub_3650E`移动continuation。
