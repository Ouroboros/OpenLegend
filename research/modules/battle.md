...
- 实现状态：`implemented_pending_review`（战斗资产/建立/回合、全部玩家动作与同步UI、AI选择/移动状态核心已接入；AI实际效果、战果/战后同步与scene结果仍按closure待实现）。
...
- `battle-closure.tsv`：0项 `pending_mapping`、24项 `pending_implementation`、57项 `implemented_pending_review`，0项最终关闭。
- B7 尚余 `sub_2DE03/sub_31C75` 联合边界；只有 battle result 实际回送 scene 后才能把两项推进到 `implemented_pending_review`。
- 下一停点：复用已完成的玩家同步相位执行AI攻击及其余AI效果，再完成结果/战后同步并把typed结果回送scene边界。
