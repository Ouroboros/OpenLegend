...
5. `sub_33599`的int16态势、候选优先级、逃跑、typed handler映射和handler后行动标记，以及`sub_33C4D..sub_34550`六个候选selector均已恢复；`sub_34AD3`休息wrapper已完整映射，`sub_34AEC`已恢复逃跑/物品重定位目的格计划，实际移动与条件休息待BattleSession同步执行；`sub_37734`方向选择/UI continuation待补；
...
- `battle-closure.tsv`：28项 `pending_mapping`、23项 `pending_implementation`、30项 `implemented_pending_review`，0项最终关闭。
- B7 尚余 `sub_2DE03/sub_31C75` 联合边界；只有 battle result 实际回送 scene 后才能把两项推进到 `implemented_pending_review`。
- 下一停点：恢复`sub_34C47/sub_3505B..sub_35372`自动攻击目标、距离与移动重选链，再推进用毒、物品、请求、暗器handler。
