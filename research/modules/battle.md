...
- `battle-closure.tsv`：0项`pending_mapping`、0项`pending_implementation`、33项`implemented_pending_review`、48项`platform_adapted`；统一最终REVIEW为48/81。
- B7 `sub_2DE03`联合边界已随typed battle result回送推进到`implemented_pending_review`。
- 下一停点：按`audit_order=49`审计`sub_37355` per-step movement；实现/测试通过不替代逐owner最终汇编↔C++ REVIEW门禁。
