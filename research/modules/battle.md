...
5. `sub_33599`的int16态势、候选优先级、逃跑、typed handler映射和handler后行动标记，以及`sub_33C4D..sub_34550`六个候选selector均已恢复；BattleSession已实际执行态势累计、战场重绘/present、延迟参数300对应的八次BIOS tick变化、selector、休息handler和逐actor后处理；`sub_34AD3`休息wrapper、`sub_3505B..sub_35372`四种攻击目标策略、`sub_355FF..sub_3570F`三个用毒目标selector已完整映射，`sub_34AEC`已恢复逃跑/物品重定位目的格计划，`sub_34C47`已恢复自动武功、装备加成、射程、移动后复检/重选的typed计划，`sub_3540E`已恢复用毒射程、移动后同目标复检和wrapped己方平均攻击回退typed计划，`sub_35803/sub_3582B`已恢复普通物品重定位后mode0使用和暗器目标/射程/移动复检/mode1使用或攻击回退typed计划，`sub_3598C`已恢复AI mode0共享物品23项状态、面板/等待参数与AI mode1暗器伤害/毒值，并按side扣队伍库存或敌方携带槽；`sub_36133`已按四个合法删除槽恢复敌方携带物品ID/数量同步左移并关闭；`sub_361AC`已按请求医疗正/零/负行动值、mode0/value0移动、目标坐标重读和自动攻击完成最终审计并关闭，`sub_36209`继续作为请求解毒共享跳入owner独立审计，`sub_36210/sub_363AC`已恢复AI医疗/解毒射程、mode1移动、同目标重检及按actor攻击与wrapped己方平均值回退攻击/休息的typed计划；`sub_3650E`已恢复mode0..3目的格、双可达性检查、最短路标记和每次一格的实际状态continuation；BattleSession现从全部AI typed handler实际进入mode0..3移动，每格提交状态、更新视图、重绘/present并等待参数40对应两次BIOS tick变化，移动后恢复原typed计划；真实battle2逃跑三格后休息并推进actor，`sub_34AEC/sub_3650E/sub_37355`为`implemented_pending_review`；自动攻击现完成目标差方向、直接/移动后area伤害、FIGHT/EFT双bank、十帧damage、提交/升级等待、体力和外层actor尾部，`sub_34C47`为`implemented_pending_review`；AI用毒也完成直接/移动后状态、11帧effect30、十帧damage、攻击/休息回退与外层尾部，`sub_3540E`推进为`implemented_pending_review`；AI医疗/解毒完成直接和移动后状态、无flash动画、攻击/休息回退与外层尾部，请求医疗/解毒也在可选mode0移动后无条件进入完整自动攻击，`sub_36209/sub_36210/sub_363AC`保持`implemented_pending_review`；AI普通物品现完成重定位、效果面板、无输入读取、九tick等待与延后来源提交；AI暗器现完成直接/移动后复检、sample13前奏、EFT后状态提交、固定kind0 damage闪烁、延后来源提交及二次超距自动攻击回退；`sub_3598C`的队伍暗器陈旧玩家payload槽BUG和随机后signed-word回绕均已恢复并关闭；玩家movement、攻击、用毒/解毒/医疗及type3/type4物品的targeting/方向、状态、FIGHT/EFT、damage、音频和行动尾部已完成；`sub_37734`额外锁定入口缓存范围、第二击动态伤害等级/cost scale、升级框与500延迟并推进为`implemented_pending_review`；
