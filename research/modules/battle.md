...
7. `sub_3AA17` 等待队尾重排为 `implemented_pending_review`；`sub_3AA4B` 已锁定重绘→present→置自动flag→AI的顺序；`sub_3AA85` 已恢复双32×32 pass、地形/overlay/物件/角色/effect/damage的typed命令计划，`BattleRenderer`已按WDX/WMP、EFT、动态FIGHT pointer基址实际执行RLE、高亮、CLOUD混色overlay、damage字体像素及MMAP物品图标，独立资产oracle与C++整帧hash一致；BattleSession已用于初始战场、actor-present、玩家动作/武功/物品菜单、movement/targeting路径光标、每个玩家/AI移动步及全部玩家攻击/支持/物品动画的重绘/present；攻击方向提示与升级框整帧hash为`0x5e46c805f42677b0`、`0x1f0048d1945a4948`，AI效果和结果调用点仍待接入；
...
- 下一停点：接玩家状态动作的共享两页角色界面，复用已完成的玩家同步相位执行AI攻击及其余AI效果，再把结果回送scene边界。
