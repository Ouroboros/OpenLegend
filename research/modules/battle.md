...
- `battle-closure.tsv`：0项`pending_mapping`、0项`pending_implementation`、34项`implemented_pending_review`、47项`platform_adapted`；统一最终REVIEW为47/81。
- B7 `sub_2DE03`联合边界已随typed battle result回送推进到`implemented_pending_review`。
- 下一停点：按`audit_order=48`审计`sub_37245` shortest-path marking；实现/测试通过不替代逐owner最终汇编↔C++ REVIEW门禁。
