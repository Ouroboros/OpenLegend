...
- 实现状态：`implemented_pending_review`（战斗资产/建立/回合、全部玩家动作与同步UI、AI选择/移动/动作0..11已接入；战果/战后同步与scene结果仍按closure待实现）。
...
- `battle-closure.tsv`：0项`pending_mapping`、9项`pending_implementation`、72项`implemented_pending_review`，0项最终关闭；按81个函数实现状态口径为88.9%。
- B7 尚余 `sub_2DE03/sub_31C75` 联合边界；只有 battle result 实际回送 scene 后才能把两项推进到 `implemented_pending_review`。
- 下一停点：完成结果/战后同步并把typed `Victory / Defeat / Escape`回送scene边界。
