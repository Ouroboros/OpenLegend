...
opcode4/`sub_2DD45` 已固定所选剧情物品 ID 的真假偏移，并以全 IDA data-xref 证明命中时的瞬时 flag 只有写入、无可观察读取；opcode13/14 的淡入、淡出 wrapper 各自形成独立同步边界；opcode16/`sub_2E29C` 扫描全部六个队伍槽，不受中间空隙截断。`sub_2DE03` 仍保持 pending：其 get-exp 参数虽已暂存，但尚未送入 B8 战斗引擎，不能以胜负偏移测试替代完整映射。
