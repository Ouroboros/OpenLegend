...
状态：B8统一最终汇编→C++ REVIEW为46/81；其余35项为`implemented_pending_review`。
...
`research/inventory/battle-closure.tsv` 以该报告为机械真值，当前0项为 `pending_mapping`、0项为 `pending_implementation`、35项为 `implemented_pending_review`、46项为`platform_adapted`。battle 区间调用到的 resource/render/input/time/random/audio 入口是共享 owner 边界，不随递归调用图吞入 battle closure。
