...
状态：B8统一最终汇编→C++ REVIEW为45/81；其余36项为`implemented_pending_review`。
...
`research/inventory/battle-closure.tsv` 以该报告为机械真值，当前0项为 `pending_mapping`、0项为 `pending_implementation`、36项为 `implemented_pending_review`、45项为`platform_adapted`。battle 区间调用到的 resource/render/input/time/random/audio 入口是共享 owner 边界，不随递归调用图吞入 battle closure。
