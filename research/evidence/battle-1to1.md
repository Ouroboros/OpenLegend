...
`research/tools/generate_b8_battle_goldens.py`只读取原版字节，不链接OpenLegend C++；两次独立生成逐字节一致后才更新正式文件，第三次生成再与正式文件逐字节相同。input-font Order38按原版启动链纠正字体来源后，全部既有像素向量由`FONT3.E16/FONT3.C16`重新派生；正式`research/evidence/battle-goldens.json`共76个顶层键，SHA256为`ab3b9a67ceec89430176a5469899e3e3ed318efc7d44a1a4947272115e77d6ab`。此前各order记录的文件级SHA256只表示当时增量版本，且其中依赖旧字体的像素证明已由本次三生成结果取代；非字体控制流、状态与RNG字段保持。

`research/tools/generate_b8_player_status_golden.py`独立读取WAR、WARFLD、WDX/WMP、HDGRP、`FONT3.E16/FONT3.C16`与palette，并从固定角色/装备/武功字节直接复算状态选择和两页像素；正式输出为`research/evidence/battle-player-status-golden.json`，SHA256为`d6d9c58f61afba15cb327da007c6e4cbd1ddc00744897aff70226fe5a8144416`，不链接或调用OpenLegend C++；两份临时结果和正式第三生成逐字节一致。
...
`sub_39B1F @ 0x39B1F..0x39B8E`已完成最终汇编→C++ REVIEW。fresh机器身份为111字节、36条指令、3个基本块、1个条件分支、0个无条件跳转、2处加载时fixup、3次direct call、唯一caller和两个本地RET；raw/loaded SHA256为`349ab646a10b3be27c00affb26813a08c15e8454ec05bcd06f1864b6cfe9ca57`与`b5814edf073c37c1db1d1b2e7507103140891cceec07247d7ded61d8aab2d24d`，全部fixup反转`0x20000`后逐字节一致。机器以signed actor→role读取`detoxification`，按向零`/15+1`得到射程，固定mode1调用独立光标；local低字恰等于1返回-1，否则调用独立`sub_39B8E(actor)`并返回0。唯一动作case3 caller忽略返回值，清栈后重读actor检查action-done。
...
`sub_3AA85` 已完成1,889 bytes/527条指令的最终入口REVIEW并归类`platform_adapted / converged_no_new_differences`。两次32×32 pass保持local-x外/local-y内；第二pass逐格按path、主cursor、副cursor、object、combatant/highlight、effect、damage排序。审计修正renderer入口错误清屏、damage遗漏`%3d`宽度及battle专用sprite错误拒绝奇数ID；修正后从入口重审全部58块、40分支、19次调用和26个caller续行。真实battle13连续视角保留4个旧像素，source/clean/retained hash为`0xbfe0bae5a6318a74`/`0x19901317cdab6f40`/`0x93fe58505f03d134`；battle4 1,157命令hash保持`0xb9f8a428699b3712`，修正后整帧/status hash为`0x5f87e9606f0c1502`/`0xf3fb3a77fd749067`。正式Golden三生成逐字节一致，SHA256=`8dafbc86668a6bc17180df43950b57c0c3f53a4a0dab9c451b46c2e86f1e5efb`。
...
`BattleSession`对mode3逐格执行render/present与两次tick，命中后执行共享用毒状态、11帧effect30、双bank音效、10帧damage及外层完成；真实battle4 seed2保持poison0→25、体力100→98、word13加1、最终RNG2993822286，首magic/damage hash为`0x47286fa4af30fce4`/`0x480732c63399ff48`。正式原资产golden双生成SHA256为`cf647cc258b768f4b0ddc1162a5f9d1b7a3364a6bb59e0624fa3805555ccf890`，Linux app Debug 14/14通过，故本owner归类`platform_adapted / converged_no_new_differences`；`sub_3570F`及其他callee继续独立待审。
...
`sub_3A8A4 @ 0x3A8A4..0x3AA17`机器身份固定为371 bytes、83条连续指令、11个基本块、5个条件分支、1个无条件跳转、24处重定位、5次direct call、两个caller和唯一`RETN @ 0x3AA16`；raw/loaded SHA256为`3097774d3ce72a87fcb377ccafc49d93976447d67ad4f909412a30a44d2085ad`与`70f7650bd3b925dd224e4f85a74b0ac0fcb6d866ca2efafa832c3b26cb18a7d2`。24处loaded DWORD均增加`0x20000`，归一化后完整371字节与只读原资产一致。入口xref是玩家动作8分派`0x333C9`与AI休息wrapper `0x34AE3`。

函数立即写actor `action_done=1`，以signed `speed/10`和round相等性选择一次必经`bounded(3)`后加3或2，体力按低16位回绕且只夹signed上界100。更新体力小于30时跳过恢复；否则以signed `physical_power/10-2`为共同bound，严格先生命后内力各调用一次RNG、加3并低字回绕，只在signed结果严格大于maximum时封顶。bound1时两个helper调用必经但不推进RNG状态。机器偶然EAX由玩家caller忽略，并虽经AI wrapper原样转发也不被其六个caller消费。

逐块对照`BattleSetup::rest_actor`及玩家/AI宿主接线后，83条指令、全部分支、RNG、共享role状态、唯一出口和两个caller均无合法域产品差异；非法actor/role安全拒绝和structured optional返回归类平台适配。新增回归锁定negative speed向零除法触发体力30、bound1两次无状态消费、体力32767回绕、HP/MP负回绕、共享role别名及非法索引不消费RNG。向量SHA256为`ae98253e95a49d58796c472bda36e0a8d4d0b3f74965034e74a5cff4448c8ac8`；Golden三生成逐字节一致且历史62键不变，正式SHA256为`2fe0a9e0eb676fa9df0bad9c735df4935caeeca90e039d117bb22b7a8cf20a23`。统一Linux Debug 14/14通过，本owner归类`platform_adapted / converged_no_new_differences`；栈探测、RNG、玩家分派、AI wrapper和统一动作尾不传播closure。

## 35. 等待队尾重排最终REVIEW

`sub_3AA17 @ 0x3AA17..0x3AA4B`机器身份固定为52 bytes、20条连续指令、4个基本块、1个条件分支、1个无条件跳转、1处重定位、2次direct call、唯一caller和`RETN @ 0x3AA4A`；raw/loaded SHA256为`a7100e83ca62a4ee61a31e9c419a9b3b44e25e6b7bcdeed0ef5a1a8fdc2d509e`与`7220b7565737cb856925c5495ebb6417fb4cf80c9957e231a56db323897bb4d6`。loaded唯一DWORD严格增加`0x20000`并可归一化为完整raw；唯一入口xref是玩家动作6分派`sub_32E59:0x333B5`。
...
## 37. 两pass战场renderer最终REVIEW

`sub_3AA85 @ 0x3AA85..0x3B1E6`机器身份固定为1,889 bytes、527条连续指令、58个IDA flow block、32个条件分支、8个无条件跳转、116处重定位、19次direct call、26个call site和唯一`RETN @ 0x3B1E5`；raw/loaded SHA256为`72c3fd2b4edc3d5e96a0d23f96a0496ded2a2cbf160473bfc33896483bc4eded`与`4f4af841cf9157f4eee76d7df295ad617092bc1abe3ee1183c66ff947b1df556`，116处DWORD均增加`0x20000`且归一化后完整匹配raw。两pass均以signed local-x为外层、local-y为内层：pass1绘ground，pass2逐格按path/主cursor/副cursor/object/combatant/effect/damage排序。range必须signed正，primary随range门，secondary只认flag1；高亮要求flag1、effect格1和HP非负，mode1/2/3着色255/47/78，非法mode不回退普通角色；damage独立于effect-visible，kind1..5使用`-%3d,-%3d,+%3d,+%3d,-%3d`和五组固定packed颜色。
...
独立原资产向量锁定连续帧source/clean/retained FNV64=`0xbfe0bae5a6318a74`/`0x19901317cdab6f40`/`0x93fe58505f03d134`及四个差异像素；battle4命令仍为1,157条和`0xb9f8a428699b3712`，damage宽度修正后整帧/status为`0x5f87e9606f0c1502`/`0xf3fb3a77fd749067`。alternate cursor、零range、负HP、非法highlight mode、effect隐藏damage、kind1..6和偶/奇同帧回归均通过。Golden三生成逐字节一致，66键SHA256=`8dafbc86668a6bc17180df43950b57c0c3f53a4a0dab9c451b46c2e86f1e5efb`，Linux app Debug 14/14通过。本owner归类`platform_adapted / converged_no_new_differences`；六个callee和所有caller不传播closure。
...
`sub_3B6BE @ 0x3B6BE..0x3BA85`机器身份固定为967 bytes、201条连续指令、54个IDA flow block、26个条件分支、6个无条件跳转、68处重定位、16次direct call、唯一caller `0x3B655`且无本地RET；raw/loaded SHA256为`88a20006a2348013356818dea6ff5d72f1ad289aabf2ee57413919a2893aa02d`与`9c6e1e546d6860fa3139e7782c57708d63deb4c9265fb44b4f6008aeb8bb4db4`。三个出口进入独立共享尾`0x39A3E`；唯一caller先拒绝非零side再把side传为参数，故真实调用的提示抑制参数恒0。
