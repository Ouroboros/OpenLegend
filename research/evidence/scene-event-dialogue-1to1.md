...
`sub_2DD77` opcode5已完成最终汇编→C++ REVIEW。入口为140字节、36条指令、3个基本块、6次call和2个本地`RET`；loaded/raw SHA256分别为`029ba93c5d02ac74cde1c9cd81457e1b7a2d5d03fa5dd57ab9bb1148b2982e50`、`224f32fa14859850cd390ce56d0e8e6a15b4e15a3c392cad7c08ddad40953cb9`，9个差异字节逐一对应9个`raw+0x20000`线性地址重定位operand。唯一caller有符号压入真假offset，固定执行`PC+3+返回偏移`。机器在当前底图依次绘制`(61,40,187,27)`面板和`(71,45)`问句，使用阴影5/前景7，present一次后才阻塞读取任意非零翻译键；仅大写Y返回真offset，其余键不经过过滤循环而立即返回假offset，按键后不额外重绘或present。全资产43次opcode5参数流SHA256为`97dfd093e7b5e3a8338317bb4f8a63820643e5795e6ea2587d339507b5c64d82`；除常见`(N,0)`外，script307 PC10和script308 PC5使用`(0,52)`反向布局。现代原文、panel、frame `0x5d8fc752d48d9a98`、真假选择和PC公式均一致；后续input owner复审修正了同批宿主keydown可在首帧present前提前回答的问题，现由`finish_presented_tick`同时门控scene与world-event问句。修正后从入口重审全部36条指令未发现第二处差异，归类`platform_adapted`；文字与present wrapper自身仍由各自closure独立终审。
