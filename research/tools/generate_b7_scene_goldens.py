...
def scene_archive_state_vectors(
    ...
    synthetic_flush_value = -32_768
    synthetic_active_buffer = bytearray(scene_maps[synthetic_active_scene])
    synthetic_active_before = struct.unpack_from(
        "<h", synthetic_active_buffer, synthetic_word_offset
    )[0]
    struct.pack_into(
    ...
    return {
        ...
        "scene_archive_swap_close_machine": {
            "entry_range": "0x29537..0x295d0",
            "size_bytes": len(scene_archive_swap_close_raw),
            "instruction_count": 38,
            "raw_function_offset": "0x22f37",
            "raw_function_sha256": sha256(scene_archive_swap_close_raw),
            "loaded_function_sha256": "e5d103d4ba35b736a4e3b4aa9143805971b94a8d7ddfc22c721f97b535092d64",
            "relocation_count": 9,
            "relocation_delta": 0x20000,
            "normalized_loaded_equals_raw": True,
            "direct_callers": ["0x2e37a", "0x2f783"],
            ...
            "group_open_mode": 0x202,
            "scene_argument": "current_scene_id",
            "write_source": "current_scene_map_buffer",
            "operation_order": ["seek", "write_full_record", "close"],
            "index_open_failure": "print_original_message_then_fatal_exit",
            "caller_result": "ignored",
            "modern_archive_ownership": "direct_snapshot_span_write_without_physical_flush",
        },
        "scene_archive_swap_flush_vector": {
            "active_scene": synthetic_active_scene,
            "layer": synthetic_layer,
            "coordinate": [synthetic_x, synthetic_y],
            "linear_tile": synthetic_linear_tile,
            "archive_record_before_value": synthetic_active_before,
            "current_buffer_value": synthetic_flush_value,
            "archive_record_before_sha256": sha256(scene_maps[synthetic_active_scene]),
            "current_buffer_sha256": sha256(bytes(synthetic_active_buffer)),
            "archive_record_after_flush_sha256": sha256(bytes(synthetic_active_buffer)),
            "snapshot_active_span_after_immediate_write_sha256": sha256(
                bytes(synthetic_active_buffer)
            ),
            "current_asset_external_opcode17_calls": external_map_writes,
            "current_asset_external_opcode38_calls": external_map_mutations,
        },
...
def dialogue_vectors(
    ...
    rest_question_input_raw = z_dat[0x27B55:0x27BE8]
    assert len(rest_question_input_raw) == 147
    assert sha256(rest_question_input_raw) == \
        "9d4b0c7d1e4ef096ee12bb3e010ca7181a9fb561b9c94eadd27d4edb78a4a5c3"
    assert rest_question_input_raw.hex() == (
        "6824000000e8bf0b0100c6056b1b030000687788030068c4070a00e8d50d0100"
        "83c4086a0468bc870c006a0068ff0000006a1b68bb0000006a286a3de829edffff"
        "83c4206a10680507000068bc870c0068c4070a006a2d6a47e87ff6000083c418ff"
        "35980b0a0068bc870c00e80bf5000083c408c6056b1b030000e85d2affff803d6b"
        "1b03005975058b442404c38b442408c3"
    )
    ...
    return {
        ...
        "question_prompts": {
            ...
            "rest_input_machine": {
                "range": "0x2E155..0x2E1E8",
                "file_offset": "0x27B55",
                "byte_count": len(rest_question_input_raw),
                "raw_sha256": sha256(rest_question_input_raw),
                "entry_last_key_clear": "0x2E15F",
                "question_present_call": "0x2E1C1",
                "pre_wait_last_key_clear": "0x2E1C9",
                "clear_and_wait_nonzero_call": "0x2E1D0",
                "uppercase_y_compare": "0x2E1D5",
                "post_key_present_calls": 0,
                "timing": "question_present_then_redundant_clear_and_first_nonzero_key_then_immediate_branch",
            },
...
def status_notice_vectors(
    ...
    opcode_45_occurrences: list[tuple[int, int, int, int]] = []
    for script_id, payload in enumerate(scripts):
        code = words(payload)
        pc = 0
        while code[pc] != -1:
            opcode = code[pc]
            if opcode == 45:
                opcode_45_occurrences.append(
                    (script_id, pc, code[pc + 1], code[pc + 2])
                )
            pc += WIDTHS[opcode]
    assert opcode_45_occurrences == [
        (484, 179, 53, 30),
        (536, 200, 49, 20),
        (581, 40, 49, 20),
    ]
    opcode_45_stream = b"".join(
        struct.pack("<IIhh", *row) for row in opcode_45_occurrences
    )
    speed_notice_infix = bytes.fromhex("20 bb b4 a5 5c bc 57 a5 5b 20")

    def render_speed_notice(role_id: int, gain: int) -> dict[str, object]:
        role = ranger[836 + role_id * 182:836 + (role_id + 1) * 182]
        role_name = record_name(role, 8)
        text = role_name + speed_notice_infix + str(gain).encode("ascii") + b"\0"
        layout_length = len(role_name) + 10
        x = 150 - (4 * layout_length + 24)
        width = 8 * layout_length + 68
        pixels = bytearray(base_frame)
    ...
    baseline_speed_notices = []
    for script_id, pc, role_id, delta in opcode_45_occurrences:
        before = struct.unpack_from("<h", ranger, 836 + role_id * 182 + 88)[0]
        after = wrapped_clamped_iq(before, delta)
        row: dict[str, object] = {
            "script": script_id,
            "pc": pc,
        ...
        if after > before:
            row.update(render_speed_notice(role_id, after - before))
        baseline_speed_notices.append(row)

    synthetic_speed_cases = []
    speed_hash_stream = bytearray()
    for before, delta in [(32767, 1), (-32768, -1), (-1, 0)]:
        after = wrapped_clamped_iq(before, delta)
        row = {"before": before, "delta": delta, "after": after, "notice": after > before}
        if after > before:
            notice = render_speed_notice(0, after - before)
            row.update(notice)
            speed_hash_stream.extend(bytes.fromhex(str(notice["frame_fnv1a64"])[2:]))
        synthetic_speed_cases.append(row)

    output["opcode_45_role_speed_panel"] = {
        "entry_range": "0x2FAB7..0x2FBC0",
        "size_bytes": 265,
        "instruction_count": 67,
        "occurrences": len(opcode_45_occurrences),
        "stream_encoding": "little_endian_<IIhh:script,pc,role_id,delta>",
        "stream_sha256": sha256(opcode_45_stream),
        "positions": [list(row) for row in opcode_45_occurrences],
        "written_role_word": "speed",
        "written_word_index": 44,
        "arithmetic": "signed_int16_wrapping_add_then_signed_clamp_0_100",
        "notice_condition": "clamped_after_greater_than_original_before",
        "notice_gain": "clamped_after_minus_original_before",
        "message_format_hex": "257320bbb4a55cbc57a55b20256400",
        "message_format_cp950": "%s 輕功增加 %d",
        "panel_layout_length": "role_name_bytes_plus_10_excluding_decimal_gain_digits",
        "notice_background": "preserve_caller_framebuffer",
        "colors": [5, 7],
        "return_value": 0,
        "program_counter_increment": 3,
        "baseline_single_call_notices": baseline_speed_notices,
        "synthetic_cases": synthetic_speed_cases,
