...
版本：v1  
状态：首轮模块与依赖冻结稿  
参考：OpenSWD3 的“架构优先、逐模块闭环”方法，但不复制其项目规模和逐函数文件化方式
...
6. 保留 `320×200` indexed framebuffer、原调色板、原资源 ID 和整数行为；窗口缩放只属于平台呈现。
...
6. `platform_sdl3`：把 palette 展开为宿主纹理并做整数缩放/宽高比处理。

不得直接把原精灵解码为 RGBA 后再把 RGBA 当游戏真值；否则难以逐像素验证调色板动画和覆盖顺序。
