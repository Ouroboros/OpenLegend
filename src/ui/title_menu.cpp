...
TitleResult TitleMenuController::handle_key(const std::uint8_t translated_key) noexcept {
    ...
    if (screen_ == TitleScreen::delete_confirmation) {
        if (translated_key == input::legacy_key::yes) {
            ...
            return {TitleCommand::delete_slot, slot_selection_};
        }
...
bool TitleMenuRenderer::render(
    ...
    case TitleScreen::delete_confirmation: {
