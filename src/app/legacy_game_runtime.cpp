...
bool LegacyGameRuntime::resume_motion_after_endpoint_presented(
    const std::chrono::nanoseconds presentation_elapsed) {
    if (!motion_tick_completed_ || !scene_motion_endpoint_waiting_ ||
    ...
    finish_completed_motion_tick();
    if (!authoritative_motion_.active()) {
        return false;
    }

    const auto remaining = time_until_motion_completion();
    if (!remaining.has_value() ||
        *remaining <= std::chrono::nanoseconds{1}) {
        return true;
    }
    const auto catch_up = std::min(
        std::max(presentation_elapsed, std::chrono::nanoseconds::zero()),
        *remaining - std::chrono::nanoseconds{1});
    advance_motion(catch_up);
    diagnostics::log_debug(
        "scene motion resumed after endpoint present sequence=" +
        std::to_string(motion_sequence()) +
        " catch_up_us=" +
        std::to_string(
            std::chrono::duration_cast<std::chrono::microseconds>(catch_up)
                .count()));
...
void LegacyGameRuntime::finish_completed_motion_tick() {
...
void LegacyGameRuntime::handle_scene_result(const scene::SceneStepResult& result) {
    ...
    case scene::SceneStepKind::open_ui: {
        update_menu_counts();
        const auto return_view = world_menu_item_event_active_
            ? LegacyGameView::world
            : LegacyGameView::scene;
        ...
        game_menu_.set_context(
            return_view == LegacyGameView::world
                ? ui::GameMenuContext::world
                : ui::GameMenuContext::scene);
        game_menu_.show_main();
        menu_return_view_ = return_view;
...
void LegacyGameRuntime::handle_menu_item_result(const ui::GameMenuResult result) {
    ...
    if (!pending_menu_item_slot_.has_value()) {
        ...
        if (item_type == 0) {
            ...
            } else if (menu_return_view_ != LegacyGameView::world ||
                       !begin_world_menu_item_event(item_id)) {
                game_menu_.show_main();
            }
            return;
        }
...
bool LegacyGameRuntime::begin_world_menu_item_event(const std::int16_t item_id) {
    auto* snapshot = game_state_.snapshot();
    if (snapshot == nullptr || world_session_ == nullptr || !retained_scene_id_.has_value()) {
        return false;
    }
    scene_session_ = std::make_unique<scene::SceneSession>(
        data_root_,
