...
- **B9 进行中**：统一最终汇编→C++ REVIEW当前为`closure=337/349`、`unique_any=274/284`、`unique_all=272/284`。`input-font`已39/39关闭，`ui`已27/39关闭。UI order26已固定`sub_2A0D9`的`reset → draw/present → selector` wrapper顺序；现代进入物品页后须等初始frame成功present才接收selector输入，Linux app Debug 14/14通过。
