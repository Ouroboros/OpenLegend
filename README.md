...
- **B9 进行中**：统一最终汇编→C++ REVIEW当前为`closure=340/349`、`unique_any=276/284`、`unique_all=275/284`。`input-font`已39/39关闭，`ui`已30/39关闭。物品菜单现按原版在确认或取消后先显示裸世界、场景或战场帧，中间选择/提示/效果页均呈现后再接收下一键，消耗品也只在效果页成功显示后扣除；正式B5 Golden三生成一致，Linux app Debug 14/14通过。
