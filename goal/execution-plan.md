...
已发布进度：`closure=236/349`，`unique_any=191/284`，`unique_all=180/284`
当前任务指针：B8 `research/inventory/battle-closure.tsv` `audit_order=2` 战斗数据载入；B8场景战斗入口已按battle owner完成最终汇编→C++ REVIEW并以独立battle golden固化
下一任务指针：B8 `audit_order=3` 战斗队列下一工作包
...
当前状态：功能实现100%，入口实现差异审计5/5已修正，Linux/Windows core/app × Debug/Release完整矩阵通过；81项closure中1项已完成统一最终汇编→C++ REVIEW，其余80项为`implemented_pending_review`，统一最终REVIEW为1/81。
