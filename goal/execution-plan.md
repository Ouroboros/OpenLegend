...
版本：v2  
当前阶段：B2 · 像素、字体与基础呈现  
当前状态：B0–B1 已完成，正在按原版汇编还原全部软件绘制原语
...
- SDL3 只用于宿主窗口、事件、时钟、音频设备和最终纹理上传，不拥有游戏语义。
- 核心像素真值为 `320×200×8-bit indexed framebuffer + 256×RGB6 palette`。
...
- SDL3 palette 展开和整数缩放；
- 固定资源的 framebuffer/palette golden hash。

验收：所有绘制原语、四向裁剪、全部当前资产帧、ASCII/Big5 字形、地图深度顺序和调色板更新逐像素符合原算法；平台缩放不得改变核心缓冲。
...
3. 当前执行 B2：逐基本块还原 framebuffer、裁剪、字体、调色板和地图呈现；
4. 按 B3–B9 顺序逐模块清零可达逻辑缺口；
5. 只有满足第 9 节全部条件后才调用 Goal 完成。
