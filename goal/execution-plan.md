...
已发布进度：`closure=151/349`，`unique_any=108/284`，`unique_all=107/284`
当前任务指针：B7 `research/inventory/scene-event-closure.tsv` `audit_order=15` 场景自动事件入口；状态为等待`audit_order=13–14`场景物品事件目标/入口工作包提交、push、TG与规则重读后开始机器码独立审计
下一任务指针：`audit_order=15` 收口后进入同表 `audit_order=16` 场景退出提示
